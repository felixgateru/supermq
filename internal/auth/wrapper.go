...
func (w wrapper) Identify(ctx context.Context, token *magistrala.IdentityReq) (res auth.IdentityRes, err error) {
	resp, err := w.auth.Identify(ctx, token)
	if err != nil {
		return auth.IdentityRes{}, err
	}
	res = auth.IdentityRes{
		ID:       resp.GetId(),
