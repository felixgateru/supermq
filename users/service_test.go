...
func TestUpdateEmail(t *testing.T) {
	...
	for _, tc := range cases {
		...
		updatedUser, err := svc.UpdateEmail(context.Background(), authn.Session{Subject: tc.reqUserID, UserID: validID, DomainID: validID}, tc.id, tc.email)
		assert.True(t, errors.Contains(err, tc.err), fmt.Sprintf("%s: expected %s got %s\n", tc.desc, tc.err, err))
...
func TestRefreshToken(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			session:     authn.Session{Subject: validID, UserID: validID, DomainID: validID},
