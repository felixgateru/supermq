...
// If the request fails, it logs the error.
func (lm *loggingMiddleware) Subscribe(ctx context.Context, key, domainID, chanRoute, subtopic string, c coap.Client) (err error) {
	defer func(begin time.Time) {
		args := []any{
			slog.String("duration", time.Since(begin).String()),
	...
	return lm.svc.Subscribe(ctx, key, domainID, chanRoute, subtopic, c)
}
...
// If the request fails, it logs the error.
func (lm *loggingMiddleware) Unsubscribe(ctx context.Context, key, domainID, chanRoute, subtopic, token string) (err error) {
	defer func(begin time.Time) {
		args := []any{
			slog.String("duration", time.Since(begin).String()),
			slog.String("channel_route", chanRoute),
	...
	return lm.svc.Unsubscribe(ctx, key, domainID, chanRoute, subtopic, token)
}
...
// If the request fails, it logs the error.
func (lm *loggingMiddleware) DisconnectHandler(ctx context.Context, domainID, chanRoute, subtopic, token string) (err error) {
	defer func(begin time.Time) {
		args := []any{
			slog.String("duration", time.Since(begin).String()),
			slog.String("channel_route", chanRoute),
	...
	return lm.svc.DisconnectHandler(ctx, domainID, chanRoute, subtopic, token)
}
