...
func (tm *tracingServiceMiddleware) Subscribe(ctx context.Context, key, domainID, chanRoute, subtopic string, c coap.Client) error {
	...
	defer span.End()
	return tm.svc.Subscribe(ctx, key, domainID, chanRoute, subtopic, c)
}

// Unsubscribe traces a CoAP unsubscribe operation.
func (tm *tracingServiceMiddleware) Unsubscribe(ctx context.Context, key, domainID, chanRoute, subtopic, token string) error {
	...
	defer span.End()
	return tm.svc.Unsubscribe(ctx, key, domainID, chanRoute, subtopic, token)
}
