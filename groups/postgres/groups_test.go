...
func TestDeleteDomainGroups(t *testing.T) {
	t.Cleanup(func() {
		_, err := db.Exec("DELETE FROM groups")
		require.Nil(t, err, fmt.Sprintf("clean groups unexpected error: %s", err))
	})
	...
	require.Nil(t, err, fmt.Sprintf("save groups unexpected error: %s", err))
	...
	cases := []struct {
		...
		{
			desc:     "remove domain groups successfully",
			domainID: domain1,
			err:      nil,
		},
		{
			desc:     "remove groups from non-existent domain",
			domainID: testsutil.GenerateUUID(t),
			err:      nil,
		},
		{
			desc:     "remove groups with invalid domain ID",
			domainID: invalidID,
			err:      nil,
		},
		{
			desc:     "remove groups with empty domain ID",
			domainID: "",
			err:      nil,
		},
	}
	...
	for _, tc := range cases {
		t.Run(tc.desc, func(t *testing.T) {
			err := repo.DeleteDomainGroups(context.Background(), tc.domainID)
			assert.True(t, errors.Contains(err, tc.err), fmt.Sprintf("%s: expected %s got %s\n", tc.desc, tc.err, err))
		})
	}
}
