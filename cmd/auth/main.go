...
func newService(ctx context.Context, db *sqlx.DB, tracer trace.Tracer, cfg config, dbConfig pgclient.Config, logger *slog.Logger, spicedbClient *authzed.ClientWithExperimental) auth.Service {
	...
	policyPa := mgagent.NewPolicyAgent(spicedbClient, logger)
	policyService := mgpolicy.NewPolicyService(policyPa)

	t := jwt.New([]byte(cfg.SecretKey))

	svc := auth.New(keysRepo, domainsRepo, idProvider, t, pa, policyService, cfg.AccessDuration, cfg.RefreshDuration, cfg.InvitationDuration)
