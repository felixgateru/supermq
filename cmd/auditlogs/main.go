...
import (
	...
	httpapi "github.com/absmach/supermq/auditlogs/api"
	"github.com/absmach/supermq/auditlogs/events"
	"github.com/absmach/supermq/auditlogs/middleware"
	auditlogspg "github.com/absmach/supermq/auditlogs/postgres"
...
const (
	svcName          = "auditlogs"
	envPrefixDB      = "SMQ_AUDIT_LOGS_DB_"
	envPrefixHTTP    = "SMQ_AUDIT_LOGS_HTTP_"
...
type config struct {
	LogLevel      string  `env:"SMQ_AUDIT_LOGS_LOG_LEVEL"   envDefault:"info"`
	ESURL         string  `env:"SMQ_ES_URL"              envDefault:"nats://localhost:4222"`
	JaegerURL     url.URL `env:"SMQ_JAEGER_URL"          envDefault:"http://localhost:4318/v1/traces"`
	SendTelemetry bool    `env:"SMQ_SEND_TELEMETRY"      envDefault:"true"`
	InstanceID    string  `env:"SMQ_AUDIT_LOGS_INSTANCE_ID" envDefault:""`
...
func main() {
	...
	db, err := pgclient.Setup(dbConfig, *auditlogspg.Migration())
	if err != nil {
		logger.Error(err.Error())
		exitCode = 1
		return
	...
	mux := chi.NewRouter()
	idp := uuid.New()
	hs := http.NewServer(ctx, cancel, svcName, httpServerConfig, httpapi.MakeHandler(svc, authn,mux, logger,  cfg.InstanceID, idp), logger)
...
func newService(db *sqlx.DB, dbConfig pgclient.Config, authz smqauthz.Authorization, logger *slog.Logger, tracer trace.Tracer) auditlogs.Service {
	database := postgres.NewDatabase(db, dbConfig, tracer)
	repo := auditlogspg.NewRepository(database)
	idp := uuid.New()

	svc := auditlogs.NewService(idp, repo)
