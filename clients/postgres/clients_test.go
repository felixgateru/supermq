...
func TestDeleteDomainClients(t *testing.T) {
	...
	_, err := repo.Save(context.Background(), client1, client2, client3)
	require.Nil(t, err, fmt.Sprintf("save clients unexpected error: %s", err))
	...
	cases := []struct {
		...
		{
			desc:     "remove domain clients successfully",
			domainID: domain1,
			err:      nil,
		},
		{
			desc:     "remove clients from non-existent domain",
			domainID: testsutil.GenerateUUID(t),
			err:      nil,
		},
		{
			desc:     "remove clients with invalid domain ID",
			domainID: invalidID,
			err:      nil,
		},
		{
			desc:     "remove clients with empty domain ID",
			domainID: "",
			err:      nil,
		},
	}
	...
	for _, tc := range cases {
		t.Run(tc.desc, func(t *testing.T) {
			err := repo.DeleteDomainClients(context.Background(), tc.domainID)
			assert.True(t, errors.Contains(err, tc.err), fmt.Sprintf("%s: expected %s got %s\n", tc.desc, tc.err, err))
		})
	}
}
