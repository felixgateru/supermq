...
func (es *eventStore) UnassignUsers(ctx context.Context, token, id string, userIds []string, relation string) error {
	err := es.svc.UnassignUsers(ctx, token, id, userIds, relation)
	if err != nil {
		return err
	}
