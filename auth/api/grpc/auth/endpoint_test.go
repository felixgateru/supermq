...
func TestIdentify(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			token: validToken,
			idt:   &grpcAuthV1.AuthNRes{Id: id, UserId: email, UserRole: uint32(auth.UserRole), DomainId: domainID},
			err:   nil,
		},
		{
	...
	for _, tc := range cases {
		t.Run(tc.desc, func(t *testing.T) {
			svcCall := svc.On("Identify", mock.Anything, mock.Anything).Return(auth.Key{Subject: id, User: email, Role: auth.UserRole, Domain: domainID}, tc.svcErr)
