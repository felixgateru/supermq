...
func (sdk mgSDK) ClientRoles(id, domainID, token string) (RolesPage, errors.SDKError) {
...
func (sdk mgSDK) ClientRoleMembers(id, roleName, domainID string, token string) ([]string, errors.SDKError) {
	url := fmt.Sprintf("%s/%s/%s/%s/%s/%s/%s", sdk.clientsURL, domainID, clientsEndpoint, id, rolesEndpoint, roleName, membersEndpoint)
	_, body, sdkerr := sdk.processRequest(http.MethodGet, url, token, nil, nil, http.StatusOK)
	if sdkerr != nil {
		return nil, sdkerr
...
func (sdk mgSDK) AvailableClientRoleActions(id, domainID, token string) ([]string, errors.SDKError) {
	url := fmt.Sprintf("%s/%s/%s/%s/%s/%s", sdk.clientsURL, domainID, clientsEndpoint, id, rolesEndpoint, "available-actions")
	_, body, sdkerr := sdk.processRequest(http.MethodGet, url, token, nil, nil, http.StatusOK)
	if sdkerr != nil {
		return nil, sdkerr
	}
