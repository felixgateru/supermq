...
func (sdk mgSDK) CreateChannels(channels []Client, domainID, token string) ([]Client, errors.SDKError) {
...
func (sdk mgSDK) ConnectClient(channelID string, clientIDs, connTypes []string, domainID, token string) errors.SDKError {
	conn := Connection{
		ClientIDs: clientIDs,
		Types:     connTypes,
	}
...
func (sdk mgSDK) DisconnectClient(channelID string, clientIDs, connTypes []string, domainID, token string) errors.SDKError {
	conn := Connection{
		ClientIDs: clientIDs,
		Types:     connTypes,
	}
...
func (sdk mgSDK) SetChannelParent(id, domainID, groupID, token string) errors.SDKError {
	...
	url := fmt.Sprintf("%s/%s/%s/%s/%s", sdk.channelsURL, domainID, clientsEndpoint, id, parentEndpoint)
