...
func (sdk mgSDK) CreateDomainRole(id, domainID string, rq RoleReq, token string) (Role, errors.SDKError) {
	data, err := json.Marshal(rq)
	if err != nil {
		return Role{}, errors.NewSDKError(err)
	}

	url := fmt.Sprintf("%s/%s/%s/%s", sdk.domainsURL, domainID, domainsEndpoint, id)
	_, body, sdkerr := sdk.processRequest(http.MethodPost, url, token, data, nil, http.StatusCreated)
	if sdkerr != nil {
		return Role{}, sdkerr
	}
...
func (sdk mgSDK) DomainRoles(id, domainID, token string) (RolesPage, errors.SDKError) {
	url := fmt.Sprintf("%s/%s/%s/%s/%s", sdk.domainsURL, domainID, domainsEndpoint, id, rolesEndpoint)
	_, body, sdkerr := sdk.processRequest(http.MethodGet, url, token, nil, nil, http.StatusOK)
	if sdkerr != nil {
...
func (sdk mgSDK) DomainRole(id, roleName, domainID, token string) (Role, errors.SDKError) {
	url := fmt.Sprintf("%s/%s/%s/%s/%s/%s", sdk.domainsURL, domainID, domainsEndpoint, id, rolesEndpoint, roleName)
	_, body, sdkerr := sdk.processRequest(http.MethodGet, url, token, nil, nil, http.StatusOK)
	if sdkerr != nil {
		return Role{}, sdkerr
	}
...
func (sdk mgSDK) UpdateDomainRole(id, roleName, newName, domainID string, token string) (Role, errors.SDKError) {
...
func (sdk mgSDK) DeleteDomainRole(id, roleName, domainID, token string) errors.SDKError {
	url := fmt.Sprintf("%s/%s/%s/%s/%s/%s", sdk.domainsURL, domainID, domainsEndpoint, id, rolesEndpoint, roleName)
...
func (sdk mgSDK) AddDomainRoleActions(id, roleName, domainID string, actions []string, token string) ([]string, errors.SDKError) {
	...
	url := fmt.Sprintf("%s/%s/%s/%s/%s/%s/%s", sdk.domainsURL, domainID, domainsEndpoint, id, rolesEndpoint, roleName, actionsEndpoint)
...
func (sdk mgSDK) DomainRoleActions(id, roleName, domainID string, token string) ([]string, errors.SDKError) {
	url := fmt.Sprintf("%s/%s/%s/%s/%s/%s/%s", sdk.domainsURL, domainID, domainsEndpoint, id, rolesEndpoint, roleName, actionsEndpoint)
	_, body, sdkerr := sdk.processRequest(http.MethodGet, url, token, nil, nil, http.StatusOK)
	if sdkerr != nil {
		return nil, sdkerr
...
func (sdk mgSDK) RemoveDomainRoleActions(id, roleName, domainID string, actions []string, token string) errors.SDKError {
	...
	url := fmt.Sprintf("%s/%s/%s/%s/%s/%s/%s/%s", sdk.domainsURL, domainID, domainsEndpoint, id, rolesEndpoint, roleName, actionsEndpoint, "delete")
	_, _, sdkerr := sdk.processRequest(http.MethodPost, url, token, data, nil, http.StatusOK)

	return sdkerr
}

func (sdk mgSDK) RemoveAllDomainRoleActions(id, roleName, domainID, token string) errors.SDKError {
	url := fmt.Sprintf("%s/%s/%s/%s/%s/%s/%s/%s", sdk.domainsURL, domainID, domainsEndpoint, id, rolesEndpoint, roleName, actionsEndpoint, "delete-all")
	_, _, sdkerr := sdk.processRequest(http.MethodDelete, url, token, nil, nil, http.StatusOK)

	return sdkerr
}

func (sdk mgSDK) AddDomainRoleMembers(id, roleName, domainID string, members []string, token string) ([]string, errors.SDKError) {
	...
	url := fmt.Sprintf("%s/%s/%s/%s/%s/%s/%s", sdk.domainsURL, domainID, domainsEndpoint, id, rolesEndpoint, roleName, membersEndpoint)
	_, body, sdkerr := sdk.processRequest(http.MethodPost, url, token, data, nil, http.StatusOK)
	if sdkerr != nil {
		return []string{}, sdkerr
	}
...
func (sdk mgSDK) DomainRoleMembers(id, roleName, domainID string, token string) ([]string, errors.SDKError) {
	url := fmt.Sprintf("%s/%s/%s/%s/%s/%s/%s", sdk.domainsURL, domainID, domainsEndpoint, id, rolesEndpoint, roleName, membersEndpoint)
	_, body, sdkerr := sdk.processRequest(http.MethodGet, url, token, nil, nil, http.StatusOK)
	if sdkerr != nil {
		return nil, sdkerr
...
func (sdk mgSDK) RemoveDomainRoleMembers(id, roleName, domainID string, members []string, token string) errors.SDKError {
	...
	url := fmt.Sprintf("%s/%s/%s/%s/%s/%s/%s/%s", sdk.domainsURL, domainID, domainsEndpoint, id, rolesEndpoint, roleName, membersEndpoint, "delete")
	_, _, sdkerr := sdk.processRequest(http.MethodPost, url, token, data, nil, http.StatusOK)

	return sdkerr
}

func (sdk mgSDK) RemoveAllDomainRoleMembers(id, roleName, domainID, token string) errors.SDKError {
	url := fmt.Sprintf("%s/%s/%s/%s/%s/%s/%s/%s", sdk.domainsURL, domainID, domainsEndpoint, id, rolesEndpoint, roleName, membersEndpoint, "delete-all")
	_, _, sdkerr := sdk.processRequest(http.MethodPost, url, token, nil, nil, http.StatusOK)

	return sdkerr
}

func (sdk mgSDK) AvailableDomainRoleActions(id, domainID, token string) ([]string, errors.SDKError) {
	url := fmt.Sprintf("%s/%s/%s/%s/%s/%s", sdk.domainsURL, domainID, domainsEndpoint, id, rolesEndpoint, "available-actions")
	_, body, sdkerr := sdk.processRequest(http.MethodGet, url, token, nil, nil, http.StatusOK)
	if sdkerr != nil {
		return nil, sdkerr
	}
