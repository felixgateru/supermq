...
func (tm *tracingMiddleware) Subscribe(ctx context.Context, sessionID, clientUsername, clientPassword, domainID, chanID, subtopic string, topicType messaging.TopicType, client *ws.Client) error {
	ctx, span := tm.tracer.Start(ctx, subscribeOP)
	defer span.End()

	return tm.svc.Subscribe(ctx, sessionID, clientUsername, clientPassword, domainID, chanID, subtopic, topicType, client)
}
