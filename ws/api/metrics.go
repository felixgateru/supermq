...
func (mm *metricsMiddleware) Subscribe(ctx context.Context, clientKey, domainID, chanRoute, subtopic string, c *ws.Client) error {
	defer func(begin time.Time) {
		mm.counter.With("method", "subscribe").Add(1)
		mm.latency.With("method", "subscribe").Observe(time.Since(begin).Seconds())
	}(time.Now())

	return mm.svc.Subscribe(ctx, clientKey, domainID, chanRoute, subtopic, c)
}
