...
func TestHandshake(t *testing.T) {
	...
	require.Nil(t, err)
	defer ts.Close()
	things.On("Authorize", mock.Anything, &magistrala.ThingsAuthReq{ThingKey: thingKey, ChannelID: id, Permission: "publish"}).Return(&magistrala.AuthorizeRes{Authorized: true, Id: "1"}, nil)
	things.On("Authorize", mock.Anything, &magistrala.ThingsAuthReq{ThingKey: thingKey, ChannelID: id, Permission: "subscribe"}).Return(&magistrala.AuthorizeRes{Authorized: true, Id: "2"}, nil)
