...
type Service interface {
	...
	Subscribe(ctx context.Context, clientKey, domainID, chanRoute, subtopic string, client *Client) error
}
...
func (svc *adapterService) Subscribe(ctx context.Context, clientKey, domainID, chanRoute, subtopic string, c *Client) error {
	if chanRoute == "" || clientKey == "" || domainID == "" {
		return svcerr.ErrAuthentication
	}

	clientID, err := svc.authorize(ctx, clientKey, domainID, chanRoute, connections.Subscribe)
	if err != nil {
...
func (svc *adapterService) authorize(ctx context.Context, clientKey, domainID, chanRoute string, msgType connections.ConnType) (string, error) {
