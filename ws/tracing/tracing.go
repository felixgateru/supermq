...
func (tm *tracingMiddleware) Subscribe(ctx context.Context, clientKey, domainID, chanTopic, subtopic string, client *ws.Client) error {
	ctx, span := tm.tracer.Start(ctx, subscribeOP)
	defer span.End()

	return tm.svc.Subscribe(ctx, clientKey, domainID, chanTopic, subtopic, client)
}
