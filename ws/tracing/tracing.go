...
func (tm *tracingMiddleware) Subscribe(ctx context.Context, clientKey, domainRoute, chanRoute, subtopic string, client *ws.Client) error {
	ctx, span := tm.tracer.Start(ctx, subscribeOP)
	defer span.End()

	return tm.svc.Subscribe(ctx, clientKey, domainRoute, chanRoute, subtopic, client)
}
